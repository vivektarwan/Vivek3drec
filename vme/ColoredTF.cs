...
namespace vme
{
    public partial class ColoredTF : UserControl
    {
        ...
        List<Knot> knots; // an array of nodes
        long[] histogram_255;
        Imagebpp iBpp;
        ...
        /* clears the histogram: control points */
        public void ResetValues()
        {
            knots.Clear();  // clear all the check points and that's enough
            paint_histogram = true;

            pp.p = new Point(marginLeft + 1, this.Height - marginBottom - 1); // establishes a single point at the origin of the default
            pp.c = System.Drawing.Color.Black;
            knots.Add(pp);
            active_number = 0;
            is_active_global = false;

            this.active.Text = "Inactively";
            this.cordX.Text = "x: ";
            this.cordY.Text = "y: ";
            this.red.Text = "R ";
            this.green.Text = "G ";
        ...
        #region Grid
        /*method draws a border and grid*/
        ...
        private void DrawAxesLabels(Graphics gr)
        {
            ...
            gr.DrawString("Brightness [bpp] and opacity [%]", f, br, p, sf);
        ...
        /* Drawing the line between control points */
        ...
        private void reset_fn_Click(object sender, EventArgs e)
        {
            if (form_this!=null)
            {
                ...
                MessageBox.Show("First download the DICOM file");
        }
        ...
        private void CheckCoordsAndPutActivePoint()
        {
            if (knots.Count > 1)
            {
                ...
                // If this is the first or the last point
                {
                    if (active_number == 0)
                    {
        ...
        private void ColoredTF_MouseClick(object sender, MouseEventArgs e) 
        {
            ...
            if (form_this != null)
            {
                ...
                if (e.Button == MouseButtons.Left)
                {
                    ...
                    {
                        if (!is_active_global && (is_active = CheckActivePoint())) // checking for hot spot
                        {
                            knots[active_number] = active_point;
                            is_active_global = true;
                            this.active.Text = "Actively";
                            this.cordX.Text = "x: " + Convert.ToString(active_point.p.X);
                            this.cordY.Text = "y: " + Convert.ToString(active_point.p.Y);
                            this.red.Text = "R " + Convert.ToString(preactive_color.R);
                            this.green.Text = "G " + Convert.ToString(preactive_color.G);
                        ...
                        {
                            // If we choose a point to edit, then simply add a new 
        ...
        /* Histogram for the current window * image * */
        private void DrawHistogram(Graphics g) 
        {
            float koeffy;
            float perc;
            int pos = marginLeft + 1;
            int py = 0;  // something that we should draw on the y axis in the form
        ...
        /* would use the transfer function of the array, and then the image */
        private void Apply_Click(object sender, EventArgs e)
        {
            ...
            if (form_this != null && !is_active_global)
            {
                ...
                for (int i = 0; i < knots.Count-1 ; i++)
                {
                    ...
                    {
                        resized_dx = dx - (marginLeft + 1); // relative zero
                        overall = knots[i + 1].p.X - knots[i].p.X; // segment between points
                ...
                if (resized_dx != 255) // Add color to the points that are to the right of the transfer function, that is, for some reason they have not been assigned a color
                {
        ...
        private void ColoredTF_Load(object sender, EventArgs e)
        {
